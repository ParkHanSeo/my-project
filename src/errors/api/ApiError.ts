...
export class ApiError<T> extends ApplicationError {
	 ...
	 * @param {number} [altStatus] - microserviceなど強制的にstatusを上書きたい場合に指定
	 * @template T
	 */
	constructor(error?: AxiosError<T>, message?: string, altStatus?: number) {
		const status = altStatus ?? error?.response?.status;
